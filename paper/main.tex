...
For each line of a program pair, we can extracted the features mentioned in Section
\ref{sec-features}.
...
Due to the large amount of data provided in this task, we only considers the
following models which are scalable to large data set.

\paragraph{Logistic Regression}
Logistic regression~(LR) is a popular linear model for classification task.
LR models the probability of a label $b$ being positive as
\[ \Pr(b = 1 | \mathbf{x}) = \frac{1}{1 + e^{-\mathbf{w}^{\top} \mathbf{x}}} \]
Where $\mathbf{w} \in \mathcal{R}^M$ is a weight vector and
$\mathbf{x} \in \mathcal{R}^M$ is the feature vector to be predicted.
LR learns a weight vector $\mathbf{w}$ by minimizing the binary crossentropy loss
over the given data set $\mathcal{D}$.
The prediction value of $\Pr(b | v)$ naturally leads to a confidence score.
%$\mathcal{C}$.
%
...
%
%While this can be somewhat mitigated by adding higher order (quadratic or
%cubic) features, this often requires substantial domain knowledge and
%computation.

\paragraph{Decision Tree}
Decision tree~(DT) learns a tree of binary predicates over the features,
recursively partitioning the feature space until a final classification can be
made.
%
Each node in the tree contains a single thresholding
$\mathbf{x}_i \leq t$ for some feature $\mathbf{x}_i$ and threshold $t$, which determines
...
\emph{Random Forest}
%
Random forest (RF)~\cite{breiman2001random} improve generalization by
training an \emph{ensemble} of distinct decision trees and using a majority
...
%
The agreement among the trees forms a natural confidence score.
%
...
called a \emph{multi-layer perceptron~(MLP)} (see \cite{Nielsen2015-pu} for
an introduction to neural networks).
%
A MLP can be represented as a directed acyclic graph whose nodes are arranged
in layers that are fully connected by weighted edges.
%
...
\begin{axis}[
    ybar,
    symbolic x coords={All, TypeError, AttributeError, ValueError},
    xtick=data,
    ymin=0,
    ylabel=Model Accuracy,
    enlarge x limits=0.5,
